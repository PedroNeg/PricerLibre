...
df_general = pd.DataFrame(columns=['ID', 'SKU', 'COSTO', 'IVA', 'Tipo publicacion', 'Estado', 'Costo Envio','Costo IIBB','Comision ML %','Precio ML','Ganancia','Ganancia %'])

df_web = pd.DataFrame(columns=['SKU','Precio WEB','Ganancia WEB', 'Ganancia WEB %'])

df_clasicos = pd.DataFrame(columns=['SKU','Ganancia ML'])

df_premium = pd.DataFrame(columns=['SKU','Ganancia ML Premium'])

df_ganweb = pd.DataFrame(columns=['SKU','Ganancia WEB %2'])
...
#---------------------------------------------------------------------------------------------------
    ...
    if preciogananciaquerida > monenviogratis:
            if costoenvio < enviominimo/1.21:
                costoenvio = enviominimo/1.21
...
#---------------------------------------------------------------------------------------------------
    ...
    if preciofinalweb > preciofinalML:
        ...
        gananciaweb = (preciofinalweb/(1+iva) - (preciofinalweb * costoventaweb) - (preciofinalweb * iibb) - costo)
        gananciaporcentajeweb = gananciaweb/preciofinalweb
        preciofinalweb = round(preciofinalweb,2)
        gananciaweb = round(gananciaweb,2)
    ...
    # EXCEL PRECIOS WEB
    # Se ingresan todos los productos calculados con sus precios WEB si son Clásicas

    #if tipopublicacion == "Clasica" and catalogo != True:
    if tipopublicacion == "Clasica":
        
        df_web = pd.concat([df_web, pd.DataFrame({
            'SKU': [codigo], 
        ...
        df_ganweb = pd.concat([df_ganweb, pd.DataFrame({
            'SKU': [codigo], 
            'Ganancia WEB %2': [gananciaporcentajeweb],

        })], ignore_index=True)

        df_clasicos = pd.concat([df_clasicos, pd.DataFrame({
            'SKU': [codigo], 
            'Ganancia ML': [gananciaporcentaje],

        })], ignore_index=True)


    if tipopublicacion == "Premium":

        df_premium = pd.concat([df_premium, pd.DataFrame({
            'SKU': [codigo], 
            'Ganancia ML Premium': [gananciaporcentaje],
...
cursor_productos.execute('SELECT Codigo FROM productos')
codigos_productos = [row[0] for row in cursor_productos.fetchall()]

# Crear un DataFrame de productos
df_productos2 = pd.DataFrame(codigos_productos, columns=['SKU'])

# Vacía df_final antes de empezar a incluirle cosas
df_final = pd.DataFrame()

# Fusionar df_productos con df_web, manteniendo todos los productos de df_productos
df_final = pd.merge(df_productos2, df_web, how='left', on='SKU')

# Introduces una columna vacía
df_final[''] = np.nan


df_final = pd.merge(df_final, df_clasicos, how='left', on='SKU')
df_final = pd.merge(df_final, df_premium, how='left', on='SKU')
df_final = pd.merge(df_final, df_ganweb, how='left', on='SKU')
...
with pd.ExcelWriter(file_path2, engine='xlsxwriter') as writer:
    ...
    # Ajustar el ancho de todas las columnas (145 pixels es aproximadamente 20 en Excel)
    ...
    worksheet.set_column('C:O', 13) # Ajustar ancho a 13 para las columnas C a O

    # Aplicar el formato de moneda a las columnas especificadas
    for column in ['C', 'G', 'H', 'J', 'K']:
        worksheet.set_column(f'{column}:{column}', 13, money_format)
    ...
    for column in ['D', 'I', 'L']:
        worksheet.set_column(f'{column}:{column}', 13, percent_format)

    # Guardar el DataFrame de los precios web en la tercera hoja
    df_final.to_excel(writer, sheet_name='Precios WEB', index=False)

    # Obtener la hoja de trabajo para cambiar el formato
    worksheet = writer.sheets['Precios WEB']

    # Ajustar el ancho de las columnas
    worksheet.set_column('A:C', 20) # Ajustar ancho a 20 para las columnas A, B y C
    
    # Aplicar el formato de moneda a las columnas especificadas
    for column in ['B', 'C']:
        worksheet.set_column(f'{column}:{column}', 20, money_format)

    # Aplicar el formato de porcentaje a las columnas especificadas
    for column in ['D']:
        worksheet.set_column(f'{column}:{column}', 20, percent_format)
